...
import { disconnectPrisma, getPrisma, withTransaction } from "../kernel/db.ts";
import { assertQueueReady, JOB_NAMES, sendInTransaction } from "../kernel/queue.ts";
 ...
 */
export async function queueSmoke(): Promise<void> {
...
if (
  scriptArg !== undefined &&
  (scriptArg.endsWith("queue-smoke.js") || scriptArg.endsWith("queue-smoke.ts"))
) {
