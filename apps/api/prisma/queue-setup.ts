...
import { JOB_RETRY_POLICY, QUEUE_SCHEMA } from "../src/kernel/queue.ts";
...
async function main(): Promise<void> {
  ...
  // pg-boss leaves maintenance timers behind that keep the event loop alive even
  // after `stop()`, so the process is ended explicitly rather than hanging.
  process.exit(0);
}
